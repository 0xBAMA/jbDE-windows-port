...
struct LineSpamConfig_t {
	ivec2 dimensions = ivec2( 3840, 2160 );
...
void LineSpamConfig_t::CreateTextures() {
	...
	// depth buffer, uint 1 channel
	textureManager->Add( "Depth Buffer", opts );

	// id buffer, uint, 2 channel
	opts.dataType		= GL_R32UI;
	textureManager->Add( "ID Buffer", opts );

	// r, g, b, count tallies, uint, 1 channel
	opts.dataType		= GL_R32UI;
...
void LineSpamConfig_t::CompileShaders() {
	...
	glObjectLabel( GL_PROGRAM, opaquePreZShader, -1, string( "Opaque PreZ" ).c_str() );

	opaqueDrawShader		= computeShader( basePath + "opaqueDraw.cs.glsl" ).shaderHandle;
	glObjectLabel( GL_PROGRAM, opaqueDrawShader, -1, string( "Opaque Draw" ).c_str() );

	transparentDrawShader	= computeShader( basePath + "transparentDraw.cs.glsl" ).shaderHandle;
	glObjectLabel( GL_PROGRAM, transparentDrawShader, -1, string( "Transparent Draw" ).c_str() );

	compositeShader			= computeShader( basePath + "composite.cs.glsl" ).shaderHandle;
	glObjectLabel( GL_PROGRAM, compositeShader, -1, string( "Composite Shader" ).c_str() );

	clearShader				= computeShader( basePath + "clear.cs.glsl" ).shaderHandle;
	glObjectLabel( GL_PROGRAM, clearShader, -1, string( "Clear Shader" ).c_str() );
}
...
void LineSpamConfig_t::PrepLineBuffers() {
	...
	std::shuffle( lines.begin(), lines.end(), rngen );
	*/
	...
	glBufferData( GL_SHADER_STORAGE_BUFFER, 4 * sizeof( vec4 ) * opaqueLines.size(), ( void * ) opaqueLines.data(), GL_STATIC_DRAW );
	...
	glBufferData( GL_SHADER_STORAGE_BUFFER, 4 * sizeof( vec4 ) * transparentLines.size(), ( void * ) transparentLines.data(), GL_STATIC_DRAW );
...
void LineSpamConfig_t::OpaquePass() {
	...
	glDispatchCompute( 64, std::max( opaqueLines.size() / 4096 + 1, size_t( 1 ) ), 1 );
	glMemoryBarrier( GL_ALL_BARRIER_BITS );
...
void LineSpamConfig_t::TransparentPass() {
	...
	glDispatchCompute( 64, std::max( transparentLines.size() / 4096 + 1, size_t( 1 ) ), 1 );
	glMemoryBarrier( GL_ALL_BARRIER_BITS );
}
