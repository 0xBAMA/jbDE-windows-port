...
// capsule bounds - primary axis is between endpoints, then some cross products to get the others
	// will also be reusing this for the capped cone, using the larger of the two radii
// ===================================================================================================
mat4 CapsuleBounds( parameters_t parameters ) {
	const vec3 pointA = vec3( parameters.data[ 1 ], parameters.data[ 2 ], parameters.data[ 3 ] );
	const vec3 pointB = vec3( parameters.data[ 4 ], parameters.data[ 5 ], parameters.data[ 6 ] );
	const vec3 midPoint = ( pointA + pointB ) / 2.0f;
	const float radius = parameters.data[ 7 ];
...
// ===================================================================================================
// capped cone
//	wasteful, copy from above, but I don't think there's a much nicer way to make this work with the mat4 passthrough approach
// ===================================================================================================
mat4 CappedConeBounds( parameters_t parameters ) {
	const vec3 pointA = vec3( parameters.data[ 1 ], parameters.data[ 2 ], parameters.data[ 3 ] );
	const vec3 pointB = vec3( parameters.data[ 4 ], parameters.data[ 5 ], parameters.data[ 6 ] );
	const vec3 midPoint = ( pointA + pointB ) / 2.0f;
	const float radius = max( parameters.data[ 7 ], parameters.data[ 8 ] ); // use the bigger of the two radii
...
// ===================================================================================================
// ellipsoid bounds, can do this basically the same way as we're doing the box bounds
// ===================================================================================================
mat4 EllipsoidBounds ( parameters_t parameters ) {
	const vec3 centerPoint = vec3( parameters.data[ 1 ], parameters.data[ 2 ], parameters.data[ 3 ] );
	const vec3 scaleFactors = vec3( parameters.data[ 4 ], parameters.data[ 5 ], parameters.data[ 6 ] );
...
void main () {
	...
	switch( int( parameters.data[ 0 ] ) ) {
		...
		case CAPPEDCONE:
			// basically same as capsule
			result = CappedConeBounds( parameters );
			break;
		...
		case ELLIPSOID:
			// basically just same as roundedbox
			result = EllipsoidBounds( parameters );
			break;
